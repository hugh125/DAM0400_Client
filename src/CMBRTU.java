public class CMBRTU {
    ...
    public static int CalculateCrc(byte[] data) {
        ...
        byte tableIndex;
        System.out.print(crcTable.length);
    ...
    public static char CalculateCrc(byte[] data, int len) {
        if (data == null)
            return 0;
        if (data.length < len) return 0;


        char crc = Character.MAX_VALUE;
        int tableIndex;

        for (int i = 0; i < len; i++)
        {
            tableIndex = 0xff & (crc ^ data[i]);
            crc >>= 8;
            crc ^= crcTable[tableIndex];
        }
        return crc;
